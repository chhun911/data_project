...
def start_file_watcher():
    ...
    # Keep the observer running until the program ends
    # No while loop needed here as the main thread has its own loop
    return observer  # Return the observer so it can be stopped from the main thread
...
try:
    ...
    while running:
        ...
        with peers_lock:
            if peers:
                logging.info(f"Current peers: {peers}")
            else:
                logging.info("No peers discovered yet")
